...
const COLUMN_DEFS = {
    ...
    webpush: [
        { id: 'cb', label: '', static: true, width: '40px' },
        { id: 'name', label: 'Chiến dịch Web Push', static: true },
        { id: 'sent', label: 'Đã gửi', type: 'number', default: true },
        { id: 'views', label: 'Lượt hiển thị', type: 'number', default: true },
        { id: 'clicks', label: 'Lượt nhấn', type: 'number', default: true },
        { id: 'ctr', label: 'CTR', type: 'percent', default: true },
        { id: 'revenue', label: 'Doanh thu', type: 'money', default: true },
        { id: 'date', label: 'Ngày gửi', default: true }
    ]
};
...
function initPageSwitching() {
    ...
    const navReportWebPush = document.getElementById('navReportWebPush');
    const navReportBeta = document.getElementById('navReportBeta');

    navReportMain?.addEventListener('click', () => switchReportPage('main'));
    navReportWebPush?.addEventListener('click', () => switchReportPage('webpush'));
...
function switchReportPage(pageType) {
    ...
    document.getElementById('reportWebPush').classList.toggle('active', pageType === 'webpush');
    ...
    document.getElementById('navReportWebPush').classList.toggle('active', pageType === 'webpush');
...
function loadTableData() {
    ...
    } else if (state.viewMode === 'webpush') {
        state.tableData = generateWebPushTableData();
    } else {
...
// ======================= WEB PUSH LOGIC =======================
function generateWebPushTableData() {
    return Array.from({ length: 20 }, (_, i) => {
        const sent = randomInt(10000, 100000);
        const views = Math.floor(sent * (randomInt(60, 90) / 100)); // 60-90% view rate
        const clicks = Math.floor(views * (randomInt(5, 20) / 100)); // 5-20% click rate
        const revenue = clicks * randomInt(50000, 200000);

        return {
            id: i + 1,
            name: `Web Push Campaign ${i + 1}`,
            sent: sent,
            views: views,
            clicks: clicks,
            ctr: ((clicks / views) * 100).toFixed(2),
            revenue: revenue,
            date: `0${randomInt(1, 9)}/12/2025`
        };
    });
}

function initWebPushReport() {
    state.viewMode = 'webpush';

    // Simulate API Load
    const data = generateWebPushTableData();
    ...
    // Update Cards
    const sentEl = document.getElementById('wp-stat-sent');
    const viewsEl = document.getElementById('wp-stat-views');
    const clicksEl = document.getElementById('wp-stat-clicks');
    const revEl = document.getElementById('wp-stat-revenue');

    if (sentEl) sentEl.innerText = formatNumber(totalSent);
    if (viewsEl) viewsEl.innerText = formatNumber(totalViews);
    if (clicksEl) clicksEl.innerText = formatNumber(totalClicks);
    if (revEl) revEl.innerText = formatAxisValue(totalRevenue) + ' VND';

    // Update Subtexts (Mock calculations)
    const viewRate = totalSent > 0 ? ((totalViews / totalSent) * 100).toFixed(1) : 0;
    const ctr = totalViews > 0 ? ((totalClicks / totalViews) * 100).toFixed(1) : 0;

    if (viewsEl) {
        const sub = viewsEl.nextElementSibling;
        if (sub) sub.innerText = `Tỷ lệ hiển thị: ${viewRate}%`;
    }
    if (clicksEl) {
        const sub = clicksEl.nextElementSibling;
        if (sub) sub.innerText = `CTR: ${ctr}%`;
    }
    ...
    // Render Charts
    renderWebPushCharts(data);
}

function renderWebPushCharts(data) {
    // Helper to create mixed chart config
    const createConfig = (labels, barData, lineData, barLabel, lineLabel) => ({
        type: 'bar',
        data: {
            labels: labels,
            datasets: [
                {
                    label: barLabel,
                    data: barData,
                    backgroundColor: '#0d47a1', // Deep FPT Blue
                    order: 2,
                    yAxisID: 'y'
                },
                {
        ...
        options: {
            ...
            maintainAspectRatio: false,
            plugins: {
                legend: { display: false } // Legend handled by HTML
            },
            scales: {
                y: {
                    beginAtZero: true,
                    grid: { borderDash: [2, 2] },
                    ticks: { callback: (val) => formatAxisValue(val) }
                },
                y1: {
                    beginAtZero: true,
                    position: 'right',
    ...
    const createRevenueConfig = (labels, barData, lineData, barLabel, lineLabel) => ({
        type: 'bar',
        data: {
            labels: labels,
            datasets: [
    ...
    // Destroy old charts
    ['wpSent', 'wpViews', 'wpClicks', 'wpRevenue'].forEach(key => {
        if (state.charts[key]) {
    ...
    // Mock Data Generator for Trend
    const labels = ['01/12', '02/12', '03/12', '04/12', '05/12', '06/12', '07/12'];

    // 1. Sent vs Success Rate
    const sentData = labels.map(() => randomInt(50000, 100000));
    const successRateData = sentData.map(() => randomInt(95, 99)); // 95-99%
    const ctxSent = document.getElementById('wpChartSent')?.getContext('2d');
    if (ctxSent) state.charts.wpSent = new Chart(ctxSent, createConfig(labels, sentData, successRateData, 'Số lượng gửi', 'Tỷ lệ thành công'));

    // 2. Views vs View Rate (Mock: View rate based on Sent)
    const viewData = sentData.map(s => Math.floor(s * (randomInt(60, 80) / 100)));
    const viewRateData = viewData.map((v, i) => Math.floor((v / sentData[i]) * 100));
    const ctxViews = document.getElementById('wpChartViews')?.getContext('2d');
    if (ctxViews) state.charts.wpViews = new Chart(ctxViews, createConfig(labels, viewData, viewRateData, 'Lượt hiển thị', 'Tỷ lệ hiển thị'));

    // 3. Clicks vs CTR
    const clickData = viewData.map(v => Math.floor(v * (randomInt(10, 25) / 100)));
    const ctrData = clickData.map((c, i) => Math.floor((c / viewData[i]) * 100));
    const ctxClicks = document.getElementById('wpChartClicks')?.getContext('2d');
    if (ctxClicks) state.charts.wpClicks = new Chart(ctxClicks, createConfig(labels, clickData, ctrData, 'Lượt nhấn', 'CTR'));

    // 4. Orders vs Revenue
    const ordersData = clickData.map(c => Math.floor(c * 0.05)); // 5% conversion
    const revenueData = ordersData.map(o => o * randomInt(500000, 1000000));
    const ctxRevenue = document.getElementById('wpChartRevenue')?.getContext('2d');
    if (ctxRevenue) state.charts.wpRevenue = new Chart(ctxRevenue, createRevenueConfig(labels, ordersData, revenueData, 'Đơn hàng', 'Doanh thu'));
}
